...
echo "=== real-8 resume-2 from resume-ckpt-75 (effective ~275) $(date) ===" >> "$REPORT"
START=$(date +%s)
./build/selfplay-zero train --out data/spz-real8.txt \
  --init data/spz-real8.txt.ckpt-75.txt \
  --iterations 625 --games 196 --schema-colors \
  --td-lambda 0.9 --threads 8 --seed 4409 --probe-every 10 \
  --probe-reps 8 \
  --telemetry build/telemetry/telemetry.jsonl \
  > build/telemetry/real8-resume2.log 2>&1
END=$(date +%s)
echo "resume wall time: $(( (END-START)/60 )) min (+ ~200 across two killed runs)" >> "$REPORT"
