...
TrainingInput make_training_input(
    ...
    result.projection =
        fq4_priority_fit::reverse_kl_i_projection(
...
ReverseKlProjection reverse_kl_i_projection(
    ...
    double ratio) {
    std::vector<fq4_priority_math::StarConstraint>
        shared_constraints;
    shared_constraints.reserve(constraints.size());
    for (const StarConstraint& constraint : constraints) {
        shared_constraints.push_back({
            .pass_index = constraint.pass_index,
            .dominated_index =
                constraint.dominated_index,
        });
    }
    fq4_priority_math::ReverseKlProjection shared =
        fq4_priority_math::reverse_kl_i_projection(
            parent_probabilities, shared_constraints,
            ratio);
    return {
        .probabilities =
            std::move(shared.probabilities),
        .active_dominated_indices =
            std::move(
                shared.active_dominated_indices),
    };
}
