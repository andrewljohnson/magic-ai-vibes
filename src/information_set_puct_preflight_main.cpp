...
load_parent() {
    ...
    if (snapshot.byte_size !=
        ...
        throw std::runtime_error(
            "ISP0 frozen C16 artifact bytes or SHA-256 "
            "drifted");
    }
    const auto artifact =
        old_school::
    ...
    if (old_school::learned_model_fingerprint(parent) !=
        ...
        throw std::runtime_error(
            "ISP0 frozen C16 model fingerprint drifted");
    }
    return parent;
}
...
int main(int argc, char* argv[]) {
    if (argc != 2 ||
        std::string_view(argv[1]) != "--run-isp0") {
        std::cerr
            << "Usage: "
            << (argc > 0
                    ? argv[0]
    ...
    try {
        ...
        const isp0::PreflightReport report =
            isp0::run_preflight(parent);
        isp0::print_report(report, std::cout);
        return report.gate_passed() ? 0 : 1;
    } catch (const std::exception& error) {
        std::cerr
            << "result=ERROR"
            << " reason=isp0_preflight_failed"
            << " message=" << error.what() << '\n';
        return 1;
    }
}
