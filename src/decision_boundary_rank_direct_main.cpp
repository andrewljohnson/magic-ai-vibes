...
// Frozen immediately after the preregistered DBC1-CACHE publication.
...
void require_artifacts_unchanged(
    ...
    if (exact_snapshot(
        ...
        exact_snapshot(
            ...
            kCacheSha256, "AQ11 cache") != cache) {
        throw std::runtime_error(
            "AQ11 frozen input changed during run");
    }
}

void print_metrics(
    std::ostream& output, std::string_view split,
    const rank::Metrics& parent,
    const rank::Metrics& candidate) {
    output
        << "metrics split=" << split
        << " deck=equal"
        << " roots=" << parent.roots
...
void print_offline(
    ...
    const rank::ModelIsolationReport& isolation,
    const rank::OfflineGate& gate) {
    output << std::fixed << std::setprecision(9);
    output
        << "AQ11-DBC2-RANK-DIRECT"
...
int main(int argc, char* argv[]) {
    if (argc != 2 ||
        ...
        std::cerr
            << "Usage: "
               "old-school-decision-boundary-rank-direct --run\n";
        return 2;
    }
    ...
    try {
        ...
        const rank::Corpus corpus =
            rank::project_corpus(source, parent);
        require_artifacts_unchanged(
            parent_artifact, cache_artifact);

        const rank::OptimizerReport fit =
            rank::optimize(corpus.train);
        const rank::OptimizerReport repeated_fit =
            rank::optimize(corpus.train);
        const rank::ModelIsolationReport isolation =
            rank::apply_delta(parent, fit.delta);
        const rank::ExactEvaluationReport exact =
            rank::evaluate_exact(
                corpus, parent, isolation.candidate,
                fit.delta);
        const rank::OfflineGate offline =
        ...
        std::cerr
            << "result=ERROR"
            << " reason=decision_boundary_rank_direct_failed"
            << " message=" << error.what() << '\n';
        return 1;
    }
}
