...
std::vector<CardId> deck_for(std::size_t index) {
    ...
    case 10: return rg_berserk_deck();
    case 11: return atog_deck();
...
int main(int argc, char** argv) {
    ...
    if (fingerprint) {
        ...
        for (unsigned seed = 0; seed < 120; ++seed) {
            hash = hash_result(
                hash, play_bot_game(BotKind::Random, seed % 13, seed));
        }
        for (unsigned seed = 0; seed < 60; ++seed) {
            hash = hash_result(
                hash,
                play_bot_game(BotKind::Handcrafted, seed % 13, seed));
        }
    ...
    if (speed) {
        {
            ...
            for (unsigned seed = 0; seed < 400; ++seed) {
                play_bot_game(BotKind::Random, seed % 13, seed);
            }
