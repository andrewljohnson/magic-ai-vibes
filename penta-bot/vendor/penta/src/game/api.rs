...
impl Game {
    ...
    /// SPZ VENDOR PATCH (proposed upstream): apply an action validated
    ...
    ///
    /// This is the same shortcut `apply_observed_action` already takes for
    /// callers holding a `PlayerObservation`, offered to callers holding
    /// just the action list -- a native search does not want to build an
    /// observation for a ply it has no choice at.
    ///
